...
func (a *app) timeLogCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  missionctl time log "Fixed the login form" --project Acme --duration 1:30
  missionctl time log "Standup" -p "Globex Portal" -d 15 --date yesterday
  missionctl time log "Client call" -p Acme -d "1 hour 20 min"`,
		Args: cobra.ExactArgs(1),
		RunE: func(_ *cobra.Command, args []string) error {
...
// reportEntry prints the entry as JSON, or as "<verb> 1:30 on Fri, Sep 25 to ● Acme: ...".
...
// oneLine is "1:30 on Fri, Sep 25 to ● Acme: Fixed the login form (id)".
